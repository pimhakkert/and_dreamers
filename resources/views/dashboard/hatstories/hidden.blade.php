                        ...
                        <form action="{{ route('hatstories.showing', $hat->hat_id) }}" method="POST" onsubmit="return confirm('Are you sure?');">
                            ...
                            <div class="hatStory-circle cursor-pointer border-brown rounded-full bg-no-repeat bg-cover bg-center block w-64 h-64 mb-5 relative" style="background-image: url(/storage/hatimage/{{ $hat->hat_image }}); border-width: 12px">
                                <div class="hatStory-hidden hidden rounded-full absolute top-0 left-0 w-full h-full" style="background-color: rgba(0, 0, 0, 0.38);">
                                    <input type="image" class="absolute" src="../images/oog-wit.svg" alt="Button to make the hat visible" style="left:50%; width: 200px; transform: translateX(-50%);">
                                    <p class="absolute text-lightbrown text-2xl" style="left: 50%; top: 68%; transform: translateX(-50%);">Show</p>
                                </div>
                            </div>
                        </form>
